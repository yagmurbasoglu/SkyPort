...
def import_nfz(path: Path, source: str, source_version: str | None, replace_source: bool) -> tuple[int, int]:
    engine = _get_engine()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Import NFZ polygons from GeoJSON into PostGIS.")
    parser.add_argument("--file", required=True, help="Path to GeoJSON FeatureCollection file.")
    parser.add_argument("--source", required=True, help="NFZ source name, e.g. AIP_ENR51.")
    parser.add_argument("--source-version", required=False, help="Source version or AIRAC tag.")
    parser.add_argument(
        "--replace-source",
        action="store_true",
        help="Delete previous rows for this source before import.",
    )

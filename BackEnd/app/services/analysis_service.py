...
def _pdf_wrapped_text(
    ...
    *,
    ...
    for line in _wrap_pdf_text(text, max_chars):
        parts.append(_pdf_text(x, current_y, line, font=font, size=size, color=color))
...
class AnalysisService:
    ...
    def delete_saved_analysis(self, *, user_id: int, analysis_id: int) -> dict[str, Any]:
        deleted = analysis_repo.delete_saved_analysis(analysis_id, user_id=user_id)
        if deleted is None:
            raise HTTPException(status_code=404, detail="Saved analysis not found.")
        return {
            "analysis_id": analysis_id,
            "message": "Saved analysis removed from profile.",
        }
